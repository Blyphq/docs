...
This guide covers Blyp behavior in production deployments: where logs are persisted, what `logger.flush()` actually guarantees, how connectors behave under sustained load, and what changes in serverless, Workers, and Convex runtimes.

Use this page together with [Configuration](/docs/configuration), [File Logging](/docs/file-logging), [Database Logging](/docs/database), [Connectors](/docs/connectors), [Next.js App Router](/docs/integrations/nextjs), [Cloudflare Workers](/docs/integrations/workers), and [Convex](/docs/integrations/convex).
...
- Convex uses a separate isolate API. Import a shared `blyp.config.ts` into `configureConvexLogger()`; file, database, delivery queues, and exception SDKs are ignored with a warning. Vendor connectors and auto OTLP targets sink from actions.
...
| Convex | console plus action-only HTTP | `@blyp/core/convex` | `logger.wrap()` flushes remote export on actions; queries and mutations cannot fetch |
...
If you are deploying to Vercel Edge, Blyp does not currently document a dedicated Edge runtime API the way it documents [Cloudflare Workers](/docs/integrations/workers) or [Convex](/docs/integrations/convex). Keep the recommendation conservative and avoid treating Edge like a writable Node runtime.
...
Convex uses `@blyp/core/convex`. Queries and mutations write `console.*` only. Actions can `fetch` vendor connectors and auto `connectors.otlp` targets from a shared `blyp.config.ts`. File logging and exception SDKs are ignored with a warning.
...
`logger.wrap()` binds `ctx` and flushes pending action OTLP, including when the handler throws. Shared configs must import `defineConfig` from `@blyp/core/config`.
...
- expect Convex to import `@blyp/core/convex`, pass a shared config into `configureConvexLogger()`, and export action logs through vendor connectors or auto OTLP targets
