...
agent:
  ...
  prerequisites: [Choose the target runtime and package manager before installing optional peers.]
  files: [package.json, pnpm-lock.yaml, bun.lock]
  commands: ["pnpm add @blyp/core"]
  sideEffects: [The package manager updates the dependency manifest and lockfile.]
  verification: [Import the selected public entrypoint and run the application's type-check or build.]
  rollback: [Remove @blyp/core and any optional peers added only for this integration.]
  failureModes:
