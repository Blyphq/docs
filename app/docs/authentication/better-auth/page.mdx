...
agent:
  ...
  appliesTo: { package: ["@blyp/core", "better-auth"], version: ">=1.6.5" }
  prerequisites: [A Better Auth instance and client already authenticate successfully.]
  files: [blyp.config.ts, src/lib/auth.ts, src/lib/auth-client.ts]
  commands: ["pnpm add @blyp/core better-auth"]
  sideEffects: [The plugins add auth lifecycle hooks and forward normalized session context.]
  verification: [Sign in and confirm server plus client-ingested logs share the expected userId and sessionId.]
  rollback: [Remove both the Blyp server and client plugins.]
  failureModes:
