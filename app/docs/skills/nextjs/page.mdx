...
Install `blyp-nextjs` after `blyp-core` when the project uses Next.js App Router. This preset tells agents to import from `blyp-js/nextjs`, wrap route handlers with `withLogger(...)`, and mount `clientLogHandler` on the exact configured ingestion path.
...
- `createLogger()` from `blyp-js/nextjs`
