...
agent:
  ...
  appliesTo: { package: ["@blyp/core", "@better-agent/core"] }
  prerequisites: [A Better Agent runtime can complete a run before instrumentation.]
  files: [blyp.config.ts, src/agents/index.ts]
  commands: ["pnpm add @blyp/core @better-agent/core"]
  sideEffects: [Opt-in capture can retain agent inputs, outputs, reasoning, or tool payloads.]
  verification: [Complete a multi-step run and confirm Blyp emits one aggregated ai_trace.]
  rollback: [Remove blypPlugin or the manual tracker and restore the original run path.]
  failureModes:
