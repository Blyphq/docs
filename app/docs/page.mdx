...
agent:
  ...
  prerequisites: [Identify whether the target runs on a server, browser, Expo, Worker, or framework adapter.]
  files: [blyp.config.ts, src/index.ts]
  commands: ["pnpm add @blyp/core"]
  sideEffects: [The first run may create local config and log output paths.]
  verification: [Emit a uniquely named info record and confirm it appears in the configured destination.]
  rollback: [Remove the Blyp initialization and restore the previous logging call.]
  failureModes:
