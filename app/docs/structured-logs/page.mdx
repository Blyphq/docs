...
- the active request `traceId` is preserved even if default structured fields contain a conflicting `traceId`

That means you can keep the same root import in standalone code and in framework routes without switching APIs.

Convex is the exception: call `logger.createStructuredLog(...)` from `@blyp/core/convex` inside Convex functions. The root `createStructuredLog` import would pull the Node logger graph into the isolate.

See [Request Tracing](/docs/working-with-blyp/request-tracing) for the full correlation model across response headers, client logs, connectors, and database rows.
