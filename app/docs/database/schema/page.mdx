...
> **Known limitation:** `@blyp/core` currently includes `traceId` in `DatabaseLogRow`, but the current CLI-generated Prisma and Drizzle schemas do not create a `trace_id` column. The generated schema is still the supported baseline today. Request trace IDs may exist in file logs and runtime records, but they are not currently persisted as a first-class database column by the generated schema.
