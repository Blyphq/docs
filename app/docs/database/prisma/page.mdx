...
agent:
  ...
  appliesTo: { package: ["@blyp/core", "prisma", "@prisma/client"] }
  prerequisites: [A Prisma datasource using PostgreSQL or MySQL already connects successfully.]
  files: [prisma/schema.prisma, blyp.config.ts]
  commands: ["blyp db:init", "blyp db:migrate", "blyp db:generate"]
  sideEffects: [A migration adds the Blyp log table and indexes.]
  verification: [Confirm prisma.blypLog exists, then emit and query a persisted log.]
  rollback: [Revert the generated migration only after preserving any retained log data.]
  failureModes:
