...
agent:
  ...
  appliesTo: { package: "@blyp/core" }
  prerequisites: [Choose Prisma, Drizzle, or MongoDB and ensure its database is reachable.]
  files: [blyp.config.ts, prisma/schema.prisma, drizzle.config.ts]
  sideEffects: [Database mode writes production log records and may require schema migrations.]
  verification: [Apply the schema, emit a test record, flush Blyp, and query the persisted row or document.]
  rollback: [Restore the previous destination and remove only Blyp-owned schema objects after preserving required data.]
  failureModes:
